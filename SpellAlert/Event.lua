...
function AlertFrame_Show(num, fullStr, mob, spell)
	if ((SAVars.MSBT == true) and (IsAddOnLoaded("MikScrollingBattleText"))) then
		if (SAVars[cType].short) then
			local SA_message = string.gsub(string.gsub(SAVars[cType].shortstr, "$m", mob), "$s", spell)
			MikSBT.DisplayMessage("" .. SA_message, nil, false, SAVars[cType].r, SAVars[cType].g, SAVars[cType].b);
		else
			MikSBT.DisplayMessage("" .. fullStr, nil, false, SAVars[cType].r, SAVars[cType].g, SAVars[cType].b);
		end
	else
		if (SAVars[cType].short) then
